...
class EvidenceReportStatusFromRequestMapperSpec extends Specification{
    private EvidenceReportStatusFromRequestMapper mapper;
    ...
    private static final EXPECTED_SICK_LEAVE_CHILD_STATUS ="co";
    ...
    void setup(){
        ...
        mapper = new EvidenceReportStatusFromRequestMapper()
    }
    ...
    def "getEvidenceReportStatusFromRequest() WHEN called with NORMAL request SHOULD return expected status"(){
        ...
        when:
        def result = mapper.getEvidenceReportStatusFromRequest(request)
        then:
