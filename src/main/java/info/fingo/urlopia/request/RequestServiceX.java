...
import info.fingo.urlopia.holidays.WorkDaysCalculator;
import info.fingo.urlopia.request.acceptance.Acceptance;
import info.fingo.urlopia.request.acceptance.AcceptanceRepositoryX;
import info.fingo.urlopia.request.acceptance.AcceptanceServiceX;
...
@Service
@Transactional
public class RequestServiceX {

    private final RequestRepositoryX requestRepository;

    private final AcceptanceRepositoryX acceptanceRepository;

    private final UserRepository userRepository;

    @Autowired
    private AcceptanceServiceX acceptanceService;

    private final HistoryLogService historyLogService;

    private final WorkDaysCalculator workDaysCalculator;

    public RequestServiceX(RequestRepositoryX requestRepository, AcceptanceRepositoryX acceptanceRepository,
    ...
    public void create(Long userId, RequestInput requestInput) {
        ...
        if (requestInput.getEndDate().isBefore(requestInput.getStartDate())) {
            throw new RuntimeException("End date id before start date");
        }
        ...
        if (request.getType() == Request.Type.OCCASIONAL) {
            ...
            user.getTeams().forEach(team -> acceptanceService.create(request, team.getLeader()));
        }
    }
    ...
    private Request createNormal(User user, RequestInput requestInput) { // thing about factory
        ...
        float hoursNeeded = workDaysCalculator.calculate(requestInput.getStartDate(), requestInput.getEndDate()) * user.getWorkTime();
    ...
    public void reject(Long requestId, Long deciderId) {
        Request request = requestRepository.findOne(requestId);
        Request.Status[] supportedStatuses = {Request.Status.PENDING};
        this.validateStatus(request.getStatus(), supportedStatuses);
