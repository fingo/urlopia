...
import info.fingo.urlopia.request.RequestServiceX;
...
@Service
@Transactional
public class AcceptanceServiceX {

    private final AcceptanceRepositoryX acceptanceRepository;

    @Autowired
    private RequestServiceX requestService;

    public AcceptanceServiceX(AcceptanceRepositoryX acceptanceRepository) {
    ...
    public void reject(Long acceptanceId) {
        ...
        Long deciderId = acceptance.getLeader().getId();
        requestService.reject(acceptance.getRequest().getId(), deciderId);
    }
